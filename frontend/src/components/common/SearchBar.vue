...
<template>
   <v-toolbar class="bg-white ma-0"
    > <v-container class="d-flex align-center"
      ...
      /> <!-- Icon buttons --> <v-btn prepend-icon="mdi-heart" stacked size="small"> MY LIST </v-btn
      > <v-btn stacked size="small" @click="goToCart"
        > <v-badge color="success" content="0"> <v-icon icon="mdi-cart"></v-icon> </v-badge> CART
        </v-btn
      > <!-- Authentication section --> <template v-if="authStore.isAuthenticated"
        > <!-- Authenticated user menu --> <v-menu
          > <template v-slot:activator="{ props }"
            > <v-btn v-bind="props" text size="small"
              > {{ authStore.user?.firstName || 'USER' }} <v-icon size="small"
          ...
          > <v-list
            > <v-list-item
              ...
              > <v-list-item-title>Tài khoản của tôi</v-list-item-title> </v-list-item
