...
<template>
   <v-app
    > <v-app-bar color="primary" dark
      > <v-btn icon @click="goBack"> <v-icon>mdi-arrow-left</v-icon> </v-btn> <v-app-bar-title
        > <v-icon class="mr-2">mdi-book-plus</v-icon> Thêm Sách Mới </v-app-bar-title
          ...
          > <v-col cols="12" md="8" lg="6"
            > <v-card class="elevation-3"
              > <v-card-title class="text-h5 pa-4"
                > <v-icon class="mr-2" color="primary">mdi-book-plus</v-icon> Thêm Sách Mới Vào Hệ
                Thống </v-card-title
              > <v-divider></v-divider> <v-card-text class="pa-4"
                ...
                > <v-form @submit.prevent="submitBook"
                  ...
                  <div class="mb-4">
                    ...
                    <div v-if="bookForm.authors.length > 0" class="mt-2">
                       <v-chip
                        ...
                        color="primary"
                        variant="elevated"
                        > <v-icon start>mdi-account</v-icon> {{ author.authorName }} </v-chip
                      >
                    </div>
                     <v-alert
                  ...
                  <div class="d-flex justify-space-between">
                     <v-btn color="grey" variant="outlined" @click="goBack" :disabled="isLoading"
                      > <v-icon start>mdi-arrow-left</v-icon> Quay lại </v-btn
                    >
                    <div class="d-flex gap-2">
                       <v-btn
                        ...
                        @click="resetForm"
                        :disabled="isLoading"
                        > <v-icon start>mdi-refresh</v-icon> Đặt lại </v-btn
                      > <v-btn
                        color="primary"
                        type="submit"
                        :loading="isLoading"
                        :disabled="
                          !bookForm.isbn || !bookForm.title || bookForm.authors.length === 0
                        "
                        > <v-icon start>mdi-content-save</v-icon> Thêm sách </v-btn
                      >
                    </div>
